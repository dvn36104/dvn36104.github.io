...
spread evenly across species, and a quick histogram makes the unevenness
visible:
...
ax.bar(counts_of_counts.index, counts_of_counts.values, color="#4e79a7")
ax.set_xlim(0.5, 30.5)
ax.set_xlabel("rows per species label")
ax.set_ylabel("number of species")
ax.set_title("Rows per species, shown to 30; the tail runs on to 784")
plt.show()
```

Every bar is an exact count on plain linear axes. The 734 single-row
species are the bar at 1. The tallest bar is at 5 rows (3,081 species),
with 4 close behind: AVONET's protocol aimed to measure at least four
birds per species, and that target is stamped straight into the shape of
the data. The x axis stops at 30 because only 262 species lie beyond it,
stretched all the way to the most sampled bird in the file, *Zosterops
chloris*, the lemon-bellied white-eye, at 784 rows. Truncating an axis is
fine; doing it silently is not, so the title says so.

The metadata explain that `data_type = 1` means a museum specimen and `data_type = 2` means
a live individual.  Therefore the 90,020 rows with a
recorded `data_type` are individual birds. The remaining 351 rows have no
`data_type` and need to be examined separately. `source` and
`specimen_number` record provenance; they do not define the row unit.

The same printout shows that `sex` is a three-level category (`M`, `F`, `U`);
 `country_wri` is the available harmonised geography; `measurer` is
an identifier for the person taking the measurements;
`publication` is provenance for measurements transcribed from published
sources and is populated for only a small subset of rows. 
...
`age` and `protocol` run between 0 and 2. These are categorical values 
