...
Start with the file itself. `.shape` gives the number of rows and
...
spread and range. Between them they answer the first screening questions
without printing a single raw row.
...
Storage types and summary numbers are the computer's view. What each
column *means* is documented: AVONET ships a metadata worksheet defining
every field, and it is served with this page alongside the data. Confirm
that the dictionary and the table describe each other, then read it.
...
That table is the data dictionary. Chart choices do not belong in it. A
