...
That table is the data dictionary. Chart choices do not belong in it. A field
does not have one universally correct aggregation, mark or scale; those
decisions depend on the question being asked.
...
The categories in that summary come from AVONET, not from us. Two fields have
no documented variable type; keep them as `not specified` until their
definitions and observed values have been checked.
...
Here are the three numeric codes in the AVONET metadata, alongside their
Pandas storage types and observed values:
