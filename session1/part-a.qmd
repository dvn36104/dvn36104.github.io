...
Allometry measures how fast one changes relative to the other.
...
```{=html}
<figure class="figure">
  <div class="row g-2 align-items-center flex-nowrap mx-auto" style="max-width: 660px;">
    <div class="col-6">
...
Start with reading the file. `.shape` gives the number of rows and
columns; `info()` reports each column's storage type and non-missing
count; `describe()` summarises every numeric column's count, centre,
spread and range. 
...
Three observations before touching anything else. First, the non-null
counts differ column by column: wing length is nearly complete at
89,275, the nares measurement stops at 55,738, and the gaps do not line
up across columns. Second, two columns (`locality` and `country`) have
**zero** observed values: entirely empty in this public release. Third,
every column is stored as one of exactly two types, text (`object`) or
decimal number (`float64`), and that split is the computer's whole
understanding of the table. Work through the two families separately.

**The text columns.** `describe` restricted to them reports, per column,
how many values are observed, how many are distinct, and the most common
...
The distinct counts split the text columns into three kinds: identifiers
with nearly as many values as rows (`avibase_id`, `specimen_number`),
names with thousands of repeats (the four taxonomy columns), and small
closed vocabularies (`source` has 80 values, `measurer` 153, `sex` just
three). For a small vocabulary the whole class list fits on screen:
...
AVONET's metadata worksheet contains a definition of every field. It is
itself a table, so inspect it the same way:
...
Six columns describing columns, 26 rows for 26 fields. The one called
`column` holds the data table's column names, which makes it the join
key between dictionary and data. If the dictionary really describes our
table, the two sets of names should be equal:

```{pyodide}
set(metadata["column"]) == set(birds.columns)
```

The dictionary and the data agree on the column list, so read the
definitions in full. Long text dies in a table cell, so print them
instead:
...
for _, row in metadata.iterrows():
    vtype = row["variable_type"] if isinstance(row["variable_type"], str) else "type not documented"
    unit = f", {row['unit']}" if isinstance(row["unit"], str) else ""
    print(f"{row['column']}  ({vtype}{unit})")
    print(textwrap.fill(str(row["description"]), width=78,
                        initial_indent="    ", subsequent_indent="    "))
    print()
```

That listing is the data dictionary. Chart choices do not belong in it. A
field does not have one universally correct aggregation, mark or scale;
those decisions depend on the question being asked.
