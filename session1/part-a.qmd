...
metadata_for_charts = metadata.assign(
    variable_type=metadata["variable_type"].fillna("not specified")
)
...
}).merge(metadata_for_charts[["column", "description", "variable_type", "unit"]],
         on="column", how="left")
...
def inspect_field(column):
    ...
    print(f"    AVONET variable type: {variable_type}")
    print(f"    unit: {unit}")
...
know how many features have each variable type. The categories come directly
from the metadata's `variable_type` column. Its two blank entries need a label
before we group and count the rows.
...
1. Start with 26 rows, one per AVONET feature, and label the two blank
   `variable_type` entries as `not specified`.
2. `groupby("variable_type")` collects rows that share the same type.
...
(alt.Chart(metadata_type_counts)
 ...
 .encode(
     ...
     tooltip=[
         alt.Tooltip("variable_type:N", title="Variable type"),
...
(alt.Chart(audit)
 ...
 .encode(
     ...
     color=alt.Color("variable_type:N", title="Variable type"),
     tooltip=[
