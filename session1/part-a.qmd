...
The `measurer` field for these rows takes on a single value, `GAPSPECIES`, repeated in all 351 rows. That is a
...
different ways. We do not calculate pixel coordinates or draw every bar
ourselves. We state what the fields mean and how values should become visual
properties; Altair works out the geometry, axes and legends.

Read an Altair specification from left to right:
...
- A **transformation** filters, groups, counts, averages or bins those rows.
  It determines what one mark in the resulting chart represents.
...
The grammar matters because these decisions are separable. Keep the data and
encodings but change the mark, and the same values take a different visual
form. Keep the mark but change `count` to `mean`, and each bar now makes a
different statistical claim. Change a field from nominal to quantitative,
and Altair constructs a different scale. The code is therefore a readable
specification of the chart's argument, not merely a set of drawing commands.
