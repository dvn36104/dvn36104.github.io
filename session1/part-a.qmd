...
First a small preparation: the metadata's `variable_type` column has two
blank entries, and a blank makes a poor group label, so copy it into a
...
1. Start with 26 rows, one per AVONET feature. `documented_type` is the
   metadata's own `variable_type`, with its two blanks filled as
   `not specified`.
