...
The default obscures the distribution in a single bar. Logging the y axis
...
untyped.columns[untyped.notna().any()].tolist()
```

Of 26 columns, only six contain anything: the five taxonomy and
identifier fields, and `measurer`. The names are what these rows exist
for; `measurer` is the odd one out, so look at what it holds.

```{pyodide}
untyped["measurer"].value_counts()
```
