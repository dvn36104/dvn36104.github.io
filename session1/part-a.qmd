...
alternative classifications needed for other analyses.We will use the BirdLife `species` field for the first
...
spread evenly across species. This first calculation uses every row in the
worksheet; below we will use `data_type` to separate measured individuals from
the extra taxonomy-only rows.

The metadata also answers the question "what does one measured row
represent?" `data_type = 1` means a museum specimen and `data_type = 2` means
a live individual. Tobias et al. likewise describe the raw morphology data as
measurements from [90,020 individual birds](https://onlinelibrary.wiley.com/doi/10.1111/ele.13898),
with duplicate measurements discounted. We should therefore call those
90,020 rows **individual birds**, not cast doubt on their unit because some
accession numbers are absent or reused. `source` and `specimen_number` record
provenance; they are not the definition of the row unit.
...
measurements. They are taxonomy links, not individual birds.
...
print(f"individual birds retained for morphology: {len(measured):,}")
```

Now the species count has a defined numerator: individual birds, excluding the
351 taxonomy-only rows. Count individuals within the documented BirdLife
`species` field.
...
pd.Series({
    "individual birds": len(measured),
    "individuals with a BirdLife species": measured["species"].notna().sum(),
    "individuals without a BirdLife species": measured["species"].isna().sum(),
    "BirdLife species with at least one individual": len(species_depth),
    "species represented by one individual": species_depth["individuals"].eq(1).sum(),
    "median individuals per species": species_depth["individuals"].median(),
    "25th percentile": species_depth["individuals"].quantile(0.25),
    "75th percentile": species_depth["individuals"].quantile(0.75),
    "maximum individuals for one species": species_depth["individuals"].max(),
})
```

There are 89,682 individual birds with a BirdLife species label, spread across
10,669 species. The median species contributes five individuals; the middle
half contribute four to nine; 383 species contribute only one; and the largest
contributes 784. We will visualise this unequal sampling depth after the Altair
introduction.
