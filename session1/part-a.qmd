...
Same data three ways. The default histogram buries every species with
fewer than ~80 rows in one bar; a log y axis rescues the tail but not
the lumping. Counting each integer value exactly shows the real shape:
the 734 single-row species at 1, a spike at 4 and 5 (AVONET's protocol
aimed for at least four birds per species), and a tail running to 784
(*Zosterops chloris*) that the panel truncates at 30 and says so.
Binning and scale are not cosmetics; they decide what a reader can see.
