...
ax.hist(rows_per_species, bins=60, color="#4e79a7")
ax.set_yscale("log")
ax.set_xlabel("rows per species label")
ax.set_ylabel("number of species (log scale)")
ax.set_title("Most species have a handful of rows; one has 784")
plt.show()
```

The y axis is logarithmic: without it, the 734 single-row species and
their neighbours would form one giant bar and everything to their right
would vanish. At the far right sits the most sampled bird in the file,
*Zosterops chloris*, the lemon-bellied white-eye, with 784 rows.
