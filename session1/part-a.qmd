...
Now make an inspection function to read the metadata definitions.
...
and interpretation. We will use the BirdLife `species` field for the first
count. How many rows and species labels are present?
...
the median is five and 734 species labels occur only once. The rows are not
spread evenly across species. At this point these are simply **rows per
label**—we have not yet established that every row is a measured bird.

#### What is a species?

A species is not a value measured from a specimen in the way that wing length
is. It is a taxonomic decision about which organisms belong to the same
evolutionary group. Under the biological species concept, a species is a group
of actually or potentially interbreeding natural populations that is
reproductively isolated from other such groups. In practice that test is
difficult for geographically separated birds,
so taxonomists also use ancestry, genetics, anatomy, plumage, song and
behaviour. Different authorities can weigh that evidence differently and
split one group into several species or lump several groups into one.

For this analysis, a "species" means a distinct name in the BirdLife taxonomy
used by this AVONET release. The count of 11,020 is therefore a count under a
specified classification, not a timeless count of natural entities. The
`species_ebird`, `ebird_species_group` and `species_birdtree` columns preserve
alternative classifications needed for other analyses.
