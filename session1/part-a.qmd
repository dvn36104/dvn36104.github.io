...
The histogram is here drawn three different ways.
The default buries every species with
fewer than ~80 rows in one bar. A log y axis makes the tail visible, but counting each integer 
value exactly shows the real shape, with
734 single-row species and a spike at 4 and 5 rows (AVONET's protocol
aimed for at least four birds per species). To make this visible, the graph is truncated at 30 rows.
...
The `unique` column sorts the text fields into kinds: identifiers with
tens of thousands of distinct values, names with thousands, and small
closed vocabularies. For a small vocabulary the whole class list with
counts fits on screen:
...
blanks from rows investigated below. Also visible in the summary:
`country_wri` is the usable geography field, `measurer` identifies who
measured, and `publication` is filled only where values were transcribed
from papers.
...
are measurements with continuous ranges in millimetres. But three
columns have suspiciously tiny ranges: `data_type` is only ever 1 or 2,
and `age` and `protocol` only 0 or 1. These look like categorical codes,
...
undocumented one. Confirm none of these rows carries a measurement.
...
Settled: 90,020 rows are individual birds (75,843 museum specimens and
14,177 live captures), and the 351 GAPSPECIES rows are taxonomy links
with no measurements. Keep the birds.
