...
ratio_chart = (
    alt.Chart(enough.reset_index()[["ratio"]])
    .mark_bar(color="#4269d0")
    .encode(
        x=alt.X("ratio:Q", bin=alt.Bin(maxbins=24),
                title="immature mean wing / adult mean wing, within species"),
        y=alt.Y("count():Q", title="species"),
    )
)

no_difference = (
    alt.Chart(pd.DataFrame({"ratio": [1.0]}))
    .mark_rule(color="#e66852", strokeWidth=1.5)
    .encode(x="ratio:Q")
)

(ratio_chart + no_difference).properties(
    width=560,
    height=200,
    title=alt.TitleParams(
        "Distribution of within-species wing length ratios",
        subtitle=f"{len(enough)} species with enough measurements of both; the red line marks no difference",
    ),
)
```

```{pyodide}
print(f"median ratio: {enough['ratio'].median():.3f}")
print(f"{(enough['ratio'] < 1).sum()} of {len(enough)} species have shorter immature wings")
```
