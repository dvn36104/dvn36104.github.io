...
#### Violin plots: a smoothed view of distribution shape
...
most 2,000 observed values from each trait. Sampling is only for drawing the
...
violin_sample = pd.concat(
    [group.sample(n=min(len(group), 2_000), random_state=36104)
...
and unevenly sampled species can dominate it. A single pooled
line can hide different trends by sex, source, protocol, or species. A useful
visualisation should state what was fitted, on which scale, to which rows, and at
...
between-species scaling signal nearly vanishes and noise dominates.
...
The effect is small but real. An analysis of
...
sample size, reminding the viewer that a difference based on two birds is not as
...
Every chart caption should name both the **grouping level** and
...
- The EDA has established that the raw worksheet contains 90,020 individual birds and 351 taxonomy-only
...
use needs **both** measurements (and a species name). Count the gaps first.
...
step, but  must be disclosed. Any figure built from this table
...
sauropsid lineage: reptiles, including birds. The term names a shared branch
of vertebrate evolution; it is not itself a variable in the dataset.
