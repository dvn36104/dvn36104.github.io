...
From the same printout: `sex` has three codes, `country_wri` is the
usable geography field, `measurer` identifies who measured, and
`publication` is filled only where values were transcribed from papers.
The 351 rows with no `data_type` get their own investigation below.
