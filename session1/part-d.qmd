...
### Shells, sub-shells and blocks

A shell is not one undivided container. Each shell is divided into
**sub-shells**, and each sub-shell contains one or more **orbitals**. An orbital
can hold at most two electrons. The four sub-shell labels used here are `s`,
`p`, `d` and `f`:

| sub-shell | number of orbitals | maximum electrons | corresponding table block |
|---|---:|---:|---|
| s | 1 | 2 | the two left-hand columns |
| p | 3 | 6 | the six right-hand columns |
| d | 5 | 10 | the ten middle columns |
| f | 7 | 14 | the two usually detached rows |

The letter is preceded by the shell number and followed by the number of
electrons occupying that sub-shell. Thus $3p^5$ means five electrons in the
p sub-shell of shell 3. The letters are traditional labels inherited from
spectroscopy; they are names, not abbreviations for four kinds of element.

A **block** is a region of the periodic table containing elements for which
that type of sub-shell is being filled. Sodium's distinguishing electron goes
into $3s$, so sodium is in the s-block. For chlorine it goes into $3p$, for
iron into $3d$, and for neodymium into $4f$, placing them in the p-, d- and
f-blocks respectively. As atomic number increases, successive elements filling
the same kind of sub-shell form a contiguous run, which becomes a rectangular
block in the standard layout. Helium is the conspicuous layout exception: its
configuration is $1s^2$, but it is placed over the noble gases because its
closed shell gives it their chemical behaviour.
...
third shell. Lithium and sodium therefore both have one electron in their
outermost shell, which is why they appear in the same column of the periodic
table.
...
The number 18 comes from the widths of the three blocks that form the main
table. A sub-shell contains a fixed number of orbitals, and each orbital can
hold two electrons:

| block | orbitals in the sub-shell | electron capacity | columns in the main table |
|---|---:|---:|---:|
| s | 1 | 2 | groups 1–2 |
| d | 5 | 10 | groups 3–12 |
| p | 3 | 6 | groups 13–18 |

So the full-width part of the table has $2 + 10 + 6 = 18$ column positions.
Period 4 makes this visible: potassium and calcium fill the two positions of
the 4s sub-shell; scandium through zinc fill the ten positions of 3d; and
gallium through krypton fill the six positions of 4p. Elements occupying the
same position in successive cycles align vertically, producing the 18 groups.

This is also why the outer-electron sawtooth above is not, by itself, a full
explanation of every group number. Across the ten transition-metal groups, an
inner d sub-shell is filling while the outermost shell can remain unchanged.
Group position therefore depends on the electron configuration, not simply on
counting the electrons in the outermost shell.

What about the f-block? An f sub-shell has seven orbitals and therefore 14
positions. In periods 6 and 7 those positions occur between the s-block and
the d-block, making each complete period $2 + 14 + 10 + 6 = 32$ elements long.
The conventional table moves those 14 positions into the two detached rows so
that the main grid remains 18 columns wide. They are not numbered as 14 extra
IUPAC groups; a 32-column table can put them back into the sequence without
changing the group numbers 1–18.
...
What are the strengths and weakenesses of the spiral approach?
...
The layout rule of the standard table is: a new row at every shell closure,
with columns collecting equal outer-shell counts.

**Rows are periods, columns are groups.**

Watch what happens with just the first 18 elements:
